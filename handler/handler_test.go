...
`
	...
	if assert.NoError(t, handler.HandleGetTasks(ctx)) {
...
`
	...
	if assert.NoError(t, handler.HandleCreateTask(ctx)) {
...
`
	...
	if assert.NoError(t, handler.HandleUpdateTask(ctx)) {
...
func Test_DeleteTask(t *testing.T) {
	...
	if assert.NoError(t, handler.HandleDeleteTask(ctx)) {
		assert.Equal(t, http.StatusOK, rec.Code)
	}
}
